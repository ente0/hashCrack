"""hashcat combinator (wordlist + mask) attack (mode -a 6) - Windows."""
import argparse
import os
import sys
...
from hashCrack.windows_inputs import (
    ...
    define_hashmode, define_workload, define_device, define_hashcat, define_force,
)
...
def build_command(hashcat_path, session, hashmode, wordlist_path, wordlist, mask_arg,
                  workload, status_timer, min_length, max_length, device, hash_file,
...
def main():
    parser = argparse.ArgumentParser(description="Combinator attack: wordlist + mask (hashcat -a 6) [Windows].")
    ...
    cmd = build_command(hashcat_path, session, hashmode, wordlist_path, wordlist, mask_arg,
                        workload, status_timer, min_length, max_length, device, hash_file,
                        plaintext_path, force)
