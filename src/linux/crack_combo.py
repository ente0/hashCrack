"""hashcat combinator (wordlist + mask) attack (mode -a 6)."""
import argparse
import os
import sys
...
from hashCrack.linux_inputs import (
    define_wordlist, define_mask, define_length, define_session, define_status,
    define_hashmode, define_workload, define_device, define_force,
)
...
def build_command(session, hashmode, wordlist_path, wordlist, mask_arg, workload,
                  status_timer, min_length, max_length, device, hash_file,
                  plaintext_path, force):
    cmd = [
        "hashcat",
        f"--session={session}",
        "-m", hashmode,
...
def main():
    parser = argparse.ArgumentParser(description="Combinator attack: wordlist + mask (hashcat -a 6).")
    ...
    cmd = build_command(session, hashmode, wordlist_path, wordlist, mask_arg,
                        workload, status_timer, min_length, max_length,
                        device, hash_file, plaintext_path, force)
