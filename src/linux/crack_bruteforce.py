"""hashcat brute-force (mask) attack (mode -a 3)."""
import argparse
import os
import sys
...
def build_command(session, hashmode, mask, workload, status_timer,
...
def main():
    parser = argparse.ArgumentParser(description="Brute-force (mask) attack (hashcat -a 3).")
    ...
    cmd = build_command(session, hashmode, mask_arg, workload, status_timer,
                        min_length, max_length, device, hash_file, plaintext_path, force)
