"""hashcat wordlist + rule attack (mode -a 0 with -r)."""
import argparse
import os
import sys
...
def build_command(session, hashmode, wordlist_path, wordlist, rule_path, rule,
...
def main():
    parser = argparse.ArgumentParser(description="Wordlist + rule attack (hashcat -a 0 -r).")
    ...
    cmd = build_command(session, hashmode, wordlist_path, wordlist, rule_path, rule,
                        workload, status_timer, device, hash_file, plaintext_path, force)
