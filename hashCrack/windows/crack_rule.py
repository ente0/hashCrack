"""hashcat wordlist + rule attack (mode -a 0 with -r) - Windows."""
import argparse
import os
import sys
...
from hashCrack.windows_inputs import (
    define_wordlist, define_rule, define_session, define_status, define_hashmode,
...
def build_command(hashcat_path, session, hashmode, wordlist_path, wordlist, rule_path, rule,
...
def main():
    parser = argparse.ArgumentParser(description="Wordlist + rule attack (hashcat -a 0 -r) [Windows].")
    ...
    cmd = build_command(hashcat_path, session, hashmode, wordlist_path, wordlist, rule_path, rule,
                        workload, status_timer, device, hash_file, plaintext_path, force)
